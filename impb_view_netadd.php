...
function impb_view_get_netadd_records(&$sql_where, $apply_limits = TRUE, $rows = '30') {
...
function impb_netadd_request_validation() {
	...
	validate_store_request_vars($filters, 'sess_impb_netadd');
	/* ================= input validation ================= */
}


function impb_view_netadd() {
	global $title, $report, $impb_search_types, $config , $netadd_actions;

   
	impb_netadd_request_validation();
    ...
    $sql_where = "";
    $netadd_results = impb_view_get_netadd_records($sql_where, true, $rows);
	...
	$nav = html_nav_bar('impb_view_netadd.php?report=bindings', MAX_DISPLAY_PAGES, get_request_var('page'), $rows, $total_rows, 14, __('NETs to add'), 'page', 'main');

	form_start('impb_view_netadd.php', 'chk');
	...
	$display_text = array(
		...
		"net_ttl" => array(__('Время действия'), "ASC"),
    ...
    if (sizeof($netadd_results) > 0) {
         foreach ($netadd_results as $netadd_result) {

            form_alternate_row('line' . $netadd_result["net_id"], true);
 			
 			form_selectable_cell((strlen(get_request_var('ip_filter')) ? preg_replace("/(" . preg_quote(get_request_var('filter')) . ")/i", "<span style='background-color: #F8D93D;'>\\1</span>", long2ip($netadd_result["net_ipaddr"])) : long2ip($netadd_result["net_ipaddr"])), $netadd_result["net_id"] );			
 			
 			form_selectable_cell((strlen(get_request_var('filter')) ? preg_replace("/(" . preg_quote(get_request_var('filter')) . ")/i", "<span style='background-color: #F8D93D;'>\\1</span>", $netadd_result["description"]) : $netadd_result["description"]), $netadd_result["net_id"] );
			
			form_selectable_cell( ($netadd_result["net_ttl"]=='0' ? "" :"До ") . $netadd_result["net_ttl_date"], $netadd_result["net_id"] );
			
			form_selectable_cell((strlen(get_request_var('filter')) ? preg_replace("/(" . preg_quote(get_request_var('filter')) . ")/i", "<span style='background-color: #F8D93D;'>\\1</span>", $netadd_result["net_description"]) : $netadd_result["net_description"]), $netadd_result["net_id"] );
			
			
			form_selectable_cell( $netadd_result["net_change_user_name"], $netadd_result["net_id"] );	
			
			form_checkbox_cell($netadd_result["net_ipaddr"], $netadd_result["net_id"]);
 			form_end_row();	
	...
	if (sizeof($netadd_results)) {
		print $nav;
	}

    impb_draw_actions_dropdown($netadd_actions, "");
	form_end();
 }
 

function form_actions_netadd() {
     global $config, $netadd_actions ;
        ...
        $selected_items = sanitize_unserialize_selected_items(get_nfilter_request_var('selected_items'));
 		...
 		if (get_request_var('drp_action') == "1") { /* удаление записи ип-мак-порт */
 		   ...
 		   foreach ($selected_items as $sel_item) {
				...
				$netadds[$cur_net_id]['net_ipaddr'] = form_input_validate(impb_translate_ip_address(get_request_var('nde_' . $cur_net_id . '_ipaddr')), 'nde_' . $cur_net_id . '_ipaddr', '^([0-2]{0,1}[0-9]{1,2}\.){3}([0-2]{0,1}[0-9]{1,2})$', false,3 );
				$netadds[$cur_net_id]['net_mask'] = form_input_validate(impb_translate_ip_address(get_request_var('nde_' . $cur_net_id . '_mask')), 'nde_' . $cur_net_id . '_mask', '^([0-2]{0,1}[0-9]{1,2}\.){3}([0-2]{0,1}[0-9]{1,2})$', false,3 );
 				$netadds[$cur_net_id]['net_description'] = form_input_validate(get_request_var('nde_' . $cur_net_id . '_description'), 'nde_' . $cur_net_id . '_description', '', true, 3);

 		   }
 	        if (!is_error_message()) {
 				if (sizeof($netadds) > 0) {
 					foreach ($netadds as $netadd) {	
 						imb_change_net_record($netadd["net_id"], $netadd["net_ipaddr"],$netadd["net_mask"],$netadd["net_description"], '1','0');
 					}
 				}
 			}
			...
			$save_data["nda_mask"] = form_input_validate(impb_translate_ip_address(get_request_var('nda_mask')), 'nda_mask', '^([0-2]{0,1}[0-9]{1,2}\.){3}([0-2]{0,1}[0-9]{1,2})$', false, 3);
 			...
 			if (!is_error_message()) {
 				...
 				$str_ids = substr($str_ids, 0, strlen($str_ids) -2);
 					imb_create_net_record($save_data["nda_ipaddr"] ,$save_data["nda_mask"], $save_data["nda_description"], '2','0');
 			}
 		}
 		
	
 	if (!is_error_message()) {
 		header("Location: impb_view_netadd.php?header=false");
 	}else{
 		header("Location: impb_view_netadd.php?header=false&action=" . get_request_var('action') . "&drp_action=" . get_request_var('drp_action') . "&post_error=" . serialize($selected_items));
	...
	form_start('impb_view_netadd.php?header=false');
		...
		if ((isset_request_var('drp_action')) && (get_request_var('drp_action') == "1")) {  /* удаление записей */
			...
			if (sizeof($netadds_rows) > 0) {
				foreach ($netadds_rows as $netadds_row) {
					...
					form_alternate_row();
						?>
						<td><?php form_hidden_box('nde_' . $netadd_id . '_id', $netadd_id, '');?></td>
						<td><?php form_text_box('nde_' . $netadd_id . '_ipaddr', $netadds_row['anet_ipaddr'], '', 15, 15, 'text', 1) ;?></td>
						<td><?php form_text_box('nde_' . $netadd_id . '_mask', $netadds_row['anet_mask'], '', 15, 15, 'text', 1) ;?></td>
						<td><?php form_text_box('nde_' . $netadd_id . '_description', $netadds_row['net_description'], '', 250, 70, 'text', 1) ;?></td>
					</tr>
					<?php
				}
			}
...
function impb_netadd_filter() {
	...
	<tr class='even'>
		...
		function applyFilter() {
			strURL  = urlPath+'plugins/impb/impb_view_netadd.php?header=false';
		...
		function clearFilter() {
			strURL  = urlPath+'plugins/impb/impb_view_netadd.php?header=false&clear=true';
			loadPageNoHeader(strURL);
		}
	
		function exportRows() {
			strURL  = urlPath+'plugins/impb/impb_view_netadd.php?export=true';
			document.location = strURL;
		}

		function importRows() {
			strURL  = urlPath+'plugins/impb/impb_view_netadd.php?import=true';
			loadPageNoHeader(strURL);
		}
